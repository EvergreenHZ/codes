...
vector<vector<int> > all_critical_path;
void get_all_critical_path(int s, vector<int> &path) {
        path.push_back(s);
        if (activity[s].size() == 0) {
                all_critical_path.push_back(path);
                return;
        } else {
                for (int i = 0; i < activity[s].size(); i++) {
                        vector<int> t_path(path);
                        int u = activity[s][i];

                        get_all_critical_path(u, t_path);
                }
        }
}

void print_all_critical_path() {
        for (int i = 0; i < n; i++) {
                if (inDegreeOrigin[i] == 0 && activity[i].size() != 0) {
                        vector<int> path;
                        get_all_critical_path(i, path);
                }
        }
        sort(all_critical_path.begin(), all_critical_path.end(), cmp);
        for (int i = 0; i < all_critical_path.size(); i++) {
                for (int j = 0; j < all_critical_path[i].size(); j++) {
                        printf("%d ", all_critical_path[i][j] + 1);
                }
                cout << endl;
        }
        return;
